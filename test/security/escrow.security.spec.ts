...
import { buildRedstonePayload, submitWithPayload, authorisedWallets } from "../helpers/redstone";
...
describe("Escrow Security", () => {
  ...
  describe("Free Balance Protection", () => {
    it("trade proceeds cannot use pending deposits", async () => {
      const { core, payment, user1, user2, owner, feePolicy } = await loadFixture(deploySecurityFixture);
  ...
  describe("Settlement Tick Clamp", () => {
    it("settlement at maxTick boundary clamps to last valid tick", async () => {
      const { core, user1, owner, feePolicy } = await loadFixture(deploySecurityFixture);
      ...
      await expect(
        core.connect(owner).finalizePrimarySettlement(marketId)
      ).to.not.be.reverted;
